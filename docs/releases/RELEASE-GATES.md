...
| Gate | Lane | Requirement | Machine evidence |
|---|---|---|---|
| `C-TOOLS` | `H1` | Every required runtime resolves to the supported content identity and passes its declared capability probe: Quarry-managed receipts for managed tools, host digests for the two distro-owned tools | Receipt/content-identity matrix, capability outcomes, template-set identity |
| `C-OUTPUT-CONTRACT` | `H1` | Empty, non-empty, malformed, truncated, non-UTF-8, partial, timeout, signal, and tool-specific exit cases map to the documented result contract | Nine typed internal receipts plus a digest-linked case matrix. Seven cases execute the candidate-bound helper; the non-empty and tool-specific-exit cases execute managed Gitleaks and require one current committed native report each. The verifier requires all nine receipts in frozen order, reconciles candidate/run ownership and receipt digests, and makes no signing or third-party attestation claim. |

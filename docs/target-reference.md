...
| `BLOCK_PRIVATE_TARGETS` | `false` | when `false`, in-scope names resolving to private / CGNAT / ULA addresses are contacted and recorded as leads. Scanner-self, loopback, link-local and metadata destinations remain protected at connect time across hostname, redirect, proxy, direct-IP, and CIDR lanes. |

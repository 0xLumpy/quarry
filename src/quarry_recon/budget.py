"""Bounded, fair, resumable processing of a lane's whole eligible input.

Throughput is bounded, membership is not: whatever a budget does not reach is persisted as a
remainder, and unbounded is the default. Design: docs/design/STEP4-SCHEDULING-DESIGN.md.
"""
...
class Budget:
    ...
    always finishes and no artifact is left half-written."""
...
def order_fairly(items, key) -> list:
    """Round-robin items across their `key(item)` groups, so each group's first item precedes any
    group's second. Deterministic."""
    groups: dict = {}
    for it in items:
...
def _fd_digest(fd) -> str:
    """sha256 of an open descriptor, from offset 0. Takes no pathname: it hashes the inode we hold, not
...
def publish_bytes(dest: Path, data: bytes, *, digest: str) -> bool:
    ...
    try:
        ...
        created = True
        # the staging directory is pinned by descriptor: its name lives in a parent anyone with write
        # access can rename, so nothing below resolves through that name again
        sfd = os.open(name, os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW, dir_fd=dfd)
        # ...and prove it is the one we just made: the name may have been swapped since the mkdir, so
        # a directory that is not ours, not private, or not empty is not staging
        st = os.fstat(sfd)
        ...
        if sfd is not None:
            try:
                ...
                pass
            # unlink the name only while it still resolves to the directory we hold; renamed away, it
...
def _already_published(dest: Path, digest: str) -> bool:
    ...
    `O_NONBLOCK`, so a planted FIFO cannot block the publisher; only a regular file may answer this."""
    try:
...
def store_writable(attempt_dir) -> bool:
    """Whether a bought page could actually be published — proven by writing, then removing the probe: an
...
def order_ranked_fair(items, *, rank, group) -> list:
    """Order by rank tier, then round-robin fairly within each tier.

    Rank decides order, never membership: a budget that stops early has done the most valuable work
    first rather than excluded anything."""
    tiers: dict = {}
    for it in items:
...
def ledger_writable(ledger) -> bool:
    """Whether completions can actually be journaled. Refuses everything `_append` refuses."""
...
def state_lock(path):
    """An exclusive, advisory, OS-released lock over one lane's project state. `flock`, not lockfile
    existence, and non-blocking, so contention raises instead of queueing."""
    import errno
    import fcntl
    path = Path(path)
    ...
    try:
        ...
        # closing the descriptor releases the lock on every exit, BaseException included: a cancelled
        # run must not leave the project locked
        fh.close()


#: only `rotation_session` holds this, so only it can build an already-locked progress map
_SESSION = object()
...
def rotation_session(state_dir, lane: str, *, schema: int, slot_grammar=None):
    ...
    Contention escapes from entering it; a `StateBusy` from inside the body is the body's own failure."""
    base = Path(state_dir)
...
#: how long a save outside a session waits for the lane lock, and how often it retries. Giving up
#: answers False rather than writing unlocked.
...
#: lane generations a refused target waits before it is asked again: a cooldown, not an exclusion
ADMISSION_COOLDOWN_GENS = 16


class RotationProgress:
    """Project-level rotation state for one lane. It orders and nothing else — losing it costs ordering
    quality, never coverage — and `done` carries the members' digest, so changed membership ranks dirty."""

    def __init__(self, path, *, lane: str, schema: int, slot_grammar=None, _session=None):
        # the configured schema is validated too: `int(True)` is 1 and `int("2")` is 2, and a schema
        # that coerces reads the rotation under the wrong meaning
    ...
    def _tuple(cls, raw, *, with_content: bool):
        ...
        # generations start at 1: a persisted 0 did not come from this map
    ...
    def _parse(cls, text: str, *, lane: str, schema: int, slot_grammar=None) -> tuple:
        """The whole state document -> (generation, targets, status, reason). Never raises, including on a
        caller-supplied grammar: anything unusable comes back as the `unusable` status with its reason."""
        try:
            doc = json.loads(text)
        ...
        for name, raw_t in raw_targets.items():
            ...
            for bucket, raw_s in raw_slots.items():
                ...
                if slot_grammar is not None:
                    # `_parse` never raises: a grammar that raises leaves the rotation unusable,
                    # not the read
                    try:
            ...
            # target-level admission records: `adm` a refusal, `adm_ok` an admission that supersedes it.
            # They order, never claim execution, and parse fail-closed.
            admission = {}
        ...
        if dropped or repaired:
            # salvaged records are not an intact rotation: work may repeat, and the driver has to be
            # able to say so
    ...
    def _read(self) -> None:
        ...
        try:
            ...
            # unreadable progress is a fresh rotation, never a stop, and the driver has to be able to say so
    ...
    def target_seq(self, target: str) -> int:
        """The reservation sequence this target was last selected at. A sequence, not a clock: a backward jump
        in wall time must not reorder the rotation."""
    ...
    def _contains(cls, parent: str, child: str) -> bool:
        """Containment is on the parsed id: `177.0` contains `177.00`, extension bits do not nest a second
        separator, and different roots are never related."""
    ...
    def _ancestors(bucket: str) -> list:
        """The containing slots of a hash-prefix id, nearest first. A flat id has none, so a lane that never
        splits is unaffected."""
    ...
    def _rank_record(self, target: str, bucket: str) -> tuple:
        """The record order may be read from, and whether it is this slot's own. An absent id inherits from
        the nearest ancestor, or after a collapse the oldest descendant. Rank only."""
    ...
    def tier(self, target: str, bucket: str, content: str) -> int:
        """0 never ran · 1 dirty (membership changed) · 2 clean · 3 refused by admission. A refusal ranks
        last, or a permanently refused target wins every lifecycle; a crash before admission stays never-
        run."""
        rec_t = self.targets.get(target, {}) or {}
        refused = rec_t.get("adm")
        if refused:
            # a later admission is a target fact and supersedes a refusal for every slot, including ones
        ...
        if not done:
            return 0
        if not own:
            # an inherited record cannot certify clean: that would claim coverage nothing produced
            return 1
    ...
    def reserve(self, target: str, bucket: str, *, at: float) -> int:
        """Take the next generation and reserve this slot with it -> the generation. Allocation belongs to the
        map, not the caller."""
    ...
    def complete(self, target: str, bucket: str, gen: int, *, at: float, content: str, members: int) -> None:
        ...
        if self._count(gen) is None or gen < 1:
            # generations start at 1, so 0 can only come from a caller that never reserved
    ...
    def reserve_batch(self, target: str, buckets, *, at: float) -> dict:
        """Reserve several slots of one target under one clock reading -> {bucket: generation}. Every member
        is validated before any generation is allocated."""
    ...
    def complete_batch(self, target: str, items, *, at: float) -> None:
        """Complete several slots of one target. Every CAS is checked before any `done` is written."""
        (when,) = self._checked(at=at)
        checked = []
        seen = set()
    ...
    def _merge_slot(mine: dict, theirs: dict) -> dict:
        """Newer generation wins, per tuple, whole: field-wise merging could assemble one slot from three
        runs."""
        out = dict(theirs)
        for key in ("res", "done"):
    ...
    def save(self) -> bool:
        ...
        if not self.held:
            ...
            if fh is None:
                return False
        try:
            # only absence means "nothing to merge with": an unreadable document is left alone rather
            # than overwritten with a rotation that lost it
            try:
                text = self.path.read_text(encoding="utf-8")
            except FileNotFoundError:
                text = None
            ...
            if text is None:
                ...
                try:
                    disk_gen, disk_targets, status, _why = self._parse(
                        text, lane=self.lane, schema=self.schema, slot_grammar=self.slot_grammar)
                    # the grammar applies to a merge too: an id dropped on load must not return
                    # through one
                except Exception:
...
def state_path(base, lane: str, config_fp: str):
    """The per-lane ledger path, namespaced by a coverage-config fingerprint — an artifact from a different
    config still validates by digest, so the fingerprint starts a clean generation instead."""
...
class Ledger:
    """A per-item record of completed work, so an interrupted or bounded run resumes. Keyed per item, not
    per work unit; completions persist, and retained evidence (including a refusal's bytes) persists too."""
    ...
    def __init__(self, state_file: Path, *, lane: str):
        ...
        self._journal_unsafe = False
        self._journal_lost = False
        self.foreign = False
        self.lost: dict[str, str] = {}        # done, but the artifact no longer verifies
    ...
    def _safe_path(self, rel) -> Path | None:
        """The artifact for `rel`, or None if it escapes the lane's directory. Containment is checked on the
        resolved path, so a symlink out is rejected too."""
    ...
    def _read_snapshot(self) -> tuple[dict, dict]:
        try:
            ...
            # invalid bytes are unusable, not absent: refuse to trust the store, never take the
            # lane down
            self.unreadable = f"state file is not valid text: {e}"
            return {}, {}
        except OSError as e:
            # unreadable is not absent: `Path.exists()` answers False for a path we may not inspect, so it
            # can never be what clears this flag
        ...
        if raw.get("lane") != self.lane:
            # a snapshot belonging to another lane marks the path foreign, and a foreign path refuses
            # every write
            self.foreign = True
            return {}, {}
    ...
    def _replay_journal(self, done: dict, digests: dict) -> None:
        """Fold appended completions over the snapshot, then repair a damaged tail of our own records. A lane
        mismatch replays nothing and rewrites nothing."""
        try:
        ...
        def _lost_record(i: int, why: str) -> None:
            """A torn tail is what a crash mid-append looks like. A bad record with intact records behind it
            means a completion was destroyed, and the store must not read that as never owned."""
        ...
        for i, line in enumerate(lines):
            ...
            if rec.get("v") != self.JOURNAL_SCHEMA or rec.get("l") != self.lane:
                # not ours: take nothing from it, and leave it alone
                self.foreign = True
                return
    ...
    def _load(self) -> None:
        ...
        for item, rel in done.items():
            ...
            if ok:
                ...
                # a validated completion is also evidence: replay restores `done` but not the evidence
                # map, so deriving it here covers a crash between journal and compaction
        ...
        # retained evidence is digest-bound too, or a tampered artifact injects fabricated findings
    ...
    def evidence(self, item: str) -> list:
        """Every validated retained artifact for this item, oldest first. A historical artifact is evidence
        only and never decides whether the item is done."""
    ...
    def artifact(self, item: str) -> Path | None:
        """The artifact this item's content lives in. May be shared: byte-identical bodies are content-
        addressed to one file, and both items point at it."""
    ...
    def artifacts(self) -> list:
        """The distinct validated artifacts — the exact set publishable to a derived tree. Anything
        else in the lane's directory is an orphan."""
        seen, out = set(), []
    ...
    def durable(self) -> bool:
        """Whether completions recorded so far survive this process. Independent of `save()`: a journalled
        completion replays on the next open even if compaction fails."""
        return not self.foreign and not self._journal_lost

    def record(self, item: str, artifact: Path, *, digest: str | None = None) -> bool:
        """Mark an item complete and bind its artifact's content. Appends to the journal; only `save()`
        re-serializes the whole map."""
        rel = str(Path(artifact).relative_to(self.path.parent))
        dig = digest or events.file_digest(artifact)
        self.done[item] = rel
        self.digests[rel] = dig
    ...
    def checkpoint(self) -> bool:
        """Prove the journal is writable, without claiming anything: the safety flags are set only by a failed
        write, and a paid caller must not spend a credit to discover one."""
...
def _remainder_tail(omitted: int, durable: bool, unretriable: int) -> str:
    """How to describe what was left: a resumable remainder, a lane that restarts, or work no later run can
    reach. Calling the third resumable is a false promise."""
    ...
    if unretriable > omitted:
        # inconsistent accounting is the caller's bug and must say so, not be clamped into a sentence
...
def report_selection(lane: str, *, measure: str, eligible: int, attempted: int, budget: Budget,
    ...
    # a budget is a cap we chose; anything else is a timeout-class gap
    ...
    # the unit must be distinct per measure: reconciliation keeps the latest per (source_id, unit), so
    # a shared one would have the outcome report overwrite this one

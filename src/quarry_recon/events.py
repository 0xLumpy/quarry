...
# The six event types (the ledger rides on a tool_finish-class update, see ledger()).
...
EVENT_TYPES = (TOOL_START, TOOL_PROGRESS, TOOL_FINISH,
               ARTIFACT_WRITTEN, COVERAGE_PARTIAL, TOOL_BLOCKED)
...
def coverage_partial(source_id, *, reason=None, produced=None) -> dict:
    """Ran but did not fully cover its input (timeout, deps-fail, jsluice --timeout-0 class)."""

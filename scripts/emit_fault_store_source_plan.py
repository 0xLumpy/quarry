#!/usr/bin/env python3
"""Emit the exact, explicitly non-promoting C-FAULT-STORE source plan."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    bodies = _inputs()
    manifest = fault_store.canonical_case_manifest_bytes()
    if bodies["fault-store-case-manifest"] != manifest:
        parser.error("committed fault-store manifest is not the frozen v1 roster")
    plan = fault_store.build_source_plan(
    ...
    args.source_plan_output.write_bytes(
        fault_store.canonical_source_plan_bytes(
            plan,
            candidate_identity_digest=args.candidate_identity_digest,
            input_bodies=bodies,
        )
